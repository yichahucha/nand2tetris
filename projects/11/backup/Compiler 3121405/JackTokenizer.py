import re,os,sys; os.chdir(sys.path[0])
...
class JackTokenizer:
    def __init__(self, file):
        ...
        # 去除注释
        jack_code = re.sub(r'//.*?\n', '', jack_code)  # 去除行注释
        jack_code = re.sub(r'/\*.*?\*/', '', jack_code, flags=re.DOTALL)  # 去除块注释
        ...
        for i, token in enumerate(tokens):
            if token in keywords:
                ...
                tokens[i] = "<integerConstant> {} </integerConstant>".format(int(token))
            elif self.isstring(token):
                tokens[i] = "<stringConstant> {} </stringConstant>".format(re.findall(r"^\"(.*?)\"$",token)[0])
            else:
