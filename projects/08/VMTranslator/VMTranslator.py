...
class Parser:
    def __init__(self,filePath):
        ...
        if os.path.isdir(filePath):  # 判断路径是否为文件夹
            vm_files = glob.glob(os.path.join(filePath, '*.vm'))  # 获取该文件夹下的所有.vm文件
        else:  # 如果是.vm文件，则直接使用该文件
            vm_files = [filePath]
    
        allCommands = []
        # 如果是文件夹添加初始化代码
        if os.path.isdir(filePath): allCommands.extend(self.code.writeInit())
    ...
    def start(self,file,fileName):
        ...
        for index,line in enumerate(VMCommands):
            ...
            if conmand[0] in self.code.symbol.memoryAccessComamands:
                ...
                assemblyComand = self.code.branching2assembly(conmand[0],conmand[1])
            elif conmand[0] == "function":
                assemblyComand = self.code.writeFunction(conmand[1],conmand[2])
            elif conmand[0] == "call":
                assemblyComand = self.code.writeCall(conmand[1],conmand[2],index)
...
class Code:
    ...
    def pushPop2assembly(self, command, segment, i):
        ...
        if segment == "constant" or segment == "local" or segment == "argument" or segment == "this" or segment == "that":
            ...
            asmStr = self.repalceMul(asmStr,{"FILENAME":self.fileName,"i":i})
